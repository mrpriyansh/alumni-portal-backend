...
const { SENDGRID_API_KEY, apiUrl } = require('./../../utils/config');
...
module.exports = async (req, res, db, emailType) => {
  ...
  const mail = {
    ...
    text: `Visit this ${apiUrl}/api/verify${emailType}Email/${token}`,
    html: `<a href="${apiUrl}/api/verify${emailType}Email/${token}"><H2>Click on this link to verify your ${emailType} email!!</H2></a>`,
  };
